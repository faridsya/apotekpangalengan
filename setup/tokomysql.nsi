...
OutFile "Installer-Pangalengan-Mysql.exe"
...
Section "Install MySQL 5"
	SetOutPath $MySQLDir\bin
		File /r "E:\farid\mysql517\bin\*.*"

	;SetOutPath $MySQLDir\lib
		;File /r "D:\mysqlmanual\lib\*.*"

		SetOutPath $MySQLDir\share
		File /r "E:\Farid\mysql517\share\*.*"

		SetOutPath $MySQLDir\data
		File /r "E:\Farid\mysql517\data\*.*"

		SetOutPath $MySQLDir\data\apotekbaleendah
		File  "apotekbaleendah\*.*"

		SetOutPath $MySQLDir
		File "E:\Farid\mysql517\*.*"
		...
		installservice:
			...
			;ExecWait '"$MySQLDir\bin\mysqld" --console'
			;ExecWait '"$MySQLDir\bin\mysqld" --defaults-file="$MySQLDir\my.ini"'
			;ExecWait '"$MySQLDir\bin\mysqld"'

	            ExecWait '"$MySQLDir\bin\mysqld" install "MySQLcoba" --defaults-file="$MySQLDir\my.ini"'

                    ;ExecWait '"$MySQLDir\bin\mysqld" --defaults-file="$MySQLDir\my.ini" --standalone'
	            ;jalankan service MySQL
	            DetailPrint "Sedang menjalankan service MySQL ..."
	            ExecWait '"$SYSDIR\net.exe" start "MySQLcoba"'
	            ...
	            ExecWait '"$MySQLDir\bin\mysqladmin.exe" -uroot password tujuh7'

	            ;menghapus user default1 (user=blank, password=blank)
	            ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7 -e "DELETE FROM mysql.user WHERE Host=$\'localhost$\' AND User=$\'$\' AND Password=$\'$\'"'
	            ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7  -e "FLUSH PRIVILEGES"'

	            ;menghapus user default2 (user=root, password=blank)
	            ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7  -e "DELETE FROM mysql.user WHERE Host=$\'127.0.0.1$\' AND User=$\'root$\' AND Password=$\'$\'"'
	            ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7  -e "FLUSH PRIVILEGES"'
	            ;set agar user root bisa login dari mesin lain (kalo diperlukan)
	           ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7  -e "GRANT ALL PRIVILEGES ON *.* TO root@$\'%$\' IDENTIFIED BY $\'root$\'"'
	            ExecWait '"$MySQLDir\bin\mysql.exe" -uroot -ptujuh7  -e "FLUSH PRIVILEGES"'
